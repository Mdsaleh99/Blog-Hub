...
export default function DashUsers() {
  ...
  const handleShowMore = async () => {
    ...
    try {
      ...
      if(res.ok){
        setUsers((prev) => [...prev, ...data.users]) // here ...prev means all the previous Users and ...data.users means all the new users and we are adding all the new users to the previous users
        if(data.users.length < 9){
          setShowMore(false)
        }
      }
